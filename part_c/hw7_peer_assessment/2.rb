...
class LineSegment < GeometryValue
  ...
  def intersectWithSegmentAsLineResult seg
    if real_close(x1, x2)  #vertical line (x1 = x2)
      if y1 < seg.y1
        seg1 = self
        seg2 = seg
      else
      ...
      if seg1.y2 > seg2.y1
        if seg1.y2 < seg2.y2 or real_close(seg1.y2, seg2.y2)
          LineSegment.new(seg2.x1, seg2.y1, seg1.x2, seg1.y2)
        else
          seg2
        end
      elsif real_close(seg1.y2, seg2.y1)
        Point.new(seg2.x1, seg2.y1)
      else
        NoPoints.new
      end
      ...
      if seg1.x2 > seg2.x1
        if seg1.x2 < seg2.x2 or real_close(seg1.x2, seg2.x2)
          LineSegment.new(seg2.x1, seg2.y1, seg1.x2, seg1.y2)
        else
          seg2
        end
      elsif real_close(seg1.x2, seg2.x1)
        Point.new(seg2.x1, seg2.y1)
      else
        NoPoints.new
      end
...
class Let < GeometryExpression
  ...
  def eval_prog env
    @e2.preprocess_prog.eval_prog(env.clone.unshift([@s, @e1.preprocess_prog.eval_prog(env)]))
  end

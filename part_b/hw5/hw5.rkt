...
;; my helper function to extend environment
(define (extend-env clo arg)
  (let* ([clo-fun (closure-fun clo)]
         [clo-env (closure-env clo)]
         [extended (cons (cons (fun-formal clo-fun)
                               arg)
                         clo-env)])
    (if (fun-nameopt clo-fun)
        (cons (cons (fun-nameopt clo-fun)
                    clo)
              extended)
        extended)))
...
(define (compute-free-vars e)
  (struct res (e fvs) #:transparent) ; result type of f (could also use a pair)
  (define (f e)
